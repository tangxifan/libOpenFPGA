...
/*
 * Get the type of a rr_graph 
 */
t_rr_graph_type RRGraph::type() const {
  return type_;
} 

/*
 * Set the type of a rr_graph 
 */
void RRGraph::set_type(t_rr_graph_type type) {
  type_ = type;
...
/*
 * Get a net id of a node in rr_graph 
 * net_id is the net that this node is mapped to 
 */
size_t RRGraph::node_net_id(RRNodeId node) const {
  VTR_ASSERT(valid_node_id(node));

  return node_net_ids_[node];
}

/*
 * Set a net id for a node in rr_graph 
 * net_id is the net that this node is mapped to 
 */
void RRGraph::set_node_net_id(RRNodeId node, size_t net_id) {
  VTR_ASSERT(valid_node_id(node));
