...
void t_rr_graph::load_rr_switch_from_arch_switch(int arch_switch_idx,
                                                 int rr_switch_idx,
                                                 int fanin,
                                                 const float R_minW_nmos,
                                                 const float R_minW_pmos) {
    ...
    rr_switch_inf[rr_switch_idx].Tdel = rr_switch_Tdel;
    rr_switch_inf[rr_switch_idx].mux_trans_size = arch_switch_inf[arch_switch_idx].mux_trans_size;
    if (arch_switch_inf[arch_switch_idx].buf_size_type == BufferSize::AUTO) {
        //Size based on resistance
        rr_switch_inf[rr_switch_idx].buf_size = trans_per_buf(arch_switch_inf[arch_switch_idx].R, R_minW_nmos, R_minW_pmos);
    } else {
        VTR_ASSERT(arch_switch_inf[arch_switch_idx].buf_size_type == BufferSize::ABSOLUTE);
        //Use the specified size
        rr_switch_inf[rr_switch_idx].buf_size = arch_switch_inf[arch_switch_idx].buf_size;
    }
    rr_switch_inf[rr_switch_idx].name = arch_switch_inf[arch_switch_idx].name;
    rr_switch_inf[rr_switch_idx].power_buffer_type = arch_switch_inf[arch_switch_idx].power_buffer_type;
    rr_switch_inf[rr_switch_idx].power_buffer_size = arch_switch_inf[arch_switch_idx].power_buffer_size;
}
...
void t_rr_graph::load_rr_switch_inf(const float R_minW_nmos, const float R_minW_pmos) {
...
/* load the global device_ctx.rr_switch_inf variable. also keep track of, for each arch switch, what
   index of the rr_switch_inf array each version of its fanin has been mapped to (through switch_fanin map) */
void t_rr_graph::load_rr_switch_inf(const int num_arch_switches, const float R_minW_nmos, const float R_minW_pmos) {
...
void t_rr_graph::load_rr_switch_from_arch_switch(
        int arch_switch_idx,
        int rr_switch_idx,
        int fanin,
        const float R_minW_nmos,
...
void t_rr_graph::remap_rr_node_switch_indices() {

    for (size_t inode = 0; inode < rr_nodes.size(); inode++) {
        auto& from_node = rr_nodes[inode];
        int num_edges = from_node.num_edges();
        for (int iedge = 0; iedge < num_edges; iedge++) {
            const t_rr_node& to_node = rr_nodes[ from_node.edge_sink_node(iedge) ];
...
void t_rr_graph::rr_graph_externals(
        const std::vector<t_segment_inf>& segment_inf, int max_chan_width,
        int wire_to_rr_ipin_switch, enum e_base_cost_type base_cost_type) {
